...
[Fact]
public async Task CrearProyecto_Funciona()
{
    var creado = await servicio.CrearProyectoAsync(peticion);
    // (sin Assert: si CrearProyectoAsync guarda mal, esta prueba PASA)
}
```
...
```csharp
[Fact]
public async Task CrearProyecto_GuardaElNombre()
{
    var creado = await servicio.CrearProyectoAsync(peticion);
